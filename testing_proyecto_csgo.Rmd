---
title: "Testing proyecto csgo"
author: "Sergio y Javi"
date: "2023-01-13"
output: html_document
---

```{r setup, include=FALSE}
#knitr::opts_chunk$set(echo = TRUE)
```

```{r}
#install.packages("ggplot2")
library(ggplot2)
library(psych)
library(caret)
library(e1071)
library(stringr)
```

Lectura Dataset
```{r}
df <- read.csv("csgo_round_snapshots.csv")
```

Este código establece una semilla aleatoria en R para que los resultados sean reproducibles, luego selecciona una muestra aleatoria de las filas de un dataframe llamado "df" y reemplaza el contenido de "df" con esa muestra seleccionada. La función "sample()" se utiliza para seleccionar las filas al azar y "nrow()" se utiliza para obtener el número total de filas en el dataframe.
```{r}
set.seed(1)
df = df[sample(1:nrow(df)), ]
```

Este código toma las primeras 10,000 filas del dataframe llamado "df" y las asigna a un nuevo dataframe llamado "df_test".
```{r}
df_test <- head(df, 10000)
```

---------------------------------------
Diferentes métricas para ver el dataset
---------------------------------------
```{r}
dim(df_test)
```

```{r}
colnames(df_test)
```

```{r}
head(df_test)
```

```{r}
summary(df_test)
```

```{r}
str(df_test)
```


Comprobar si existe duplicidad en el dataset
Este código utiliza la función any() y duplicated() para determinar si hay algún valor duplicado en el dataframe "df_test". La función duplicated() identifica los valores duplicados en el dataframe y devuelve un vector lógico con el mismo número de filas que el dataframe original, donde cada valor verdadero indica que el valor en esa fila es un duplicado. La función any() toma un vector lógico como entrada y devuelve verdadero si al menos uno de los valores es verdadero y falso si todos los valores son falsos. En este caso, el código devuelve verdadero si hay algún valor duplicado en el dataframe "df_test" y falso si no hay valores duplicados.
```{r}
any(duplicated(df_test))
```

Como muestra "TRUE" significa que existen datos duplicados, vamos a quitarlos.
Este código utiliza la función duplicated() para seleccionar las filas con valores duplicados en el dataframe "df_test" y devuelve esas filas como un nuevo dataframe. La función duplicated() identifica los valores duplicados en el dataframe y devuelve un vector lógico con el mismo número de filas que el dataframe original, donde cada valor verdadero indica que el valor en esa fila es un duplicado. Entonces, utilizando el vector lógico devuelto por duplicated() como un índice, se seleccionan solo las filas con valores duplicados en el dataframe "df_test" y se devuelve como un nuevo dataframe.
```{r}
df_test[duplicated(df_test), ]
```

Este código utiliza la función unique() para eliminar los valores duplicados en el dataframe "df_test" y asigna el dataframe sin duplicados a un nuevo dataframe llamado "df_test". La función unique() toma un dataframe como entrada y devuelve un dataframe con solo los valores únicos.

Por otro lado, la función dim() devuelve las dimensiones del dataframe, es decir, el número de filas y columnas. En este caso, el código devuelve el número de filas y columnas del nuevo dataframe "df_test" sin duplicados.
```{r}
df_test <- unique(df_test)
dim(df_test)
```

Renombre de casillas de la columna "map" para posteriormente hacer gráficas.
```{r}
# remover "de_" de las casillas de la columna "col_name"
df_test$map <- sub("de_","", df_test$map)

# convertir la primera letra de cada casilla a mayúscula
df_test$map <- str_to_title(df_test$map)
```

Borrar todas las filas en las que existan rondas de desempate.
Borrar todas las filas que contengan el mapa "Cache".
Columna "ct_score" y "t_score" con valores > 15.

Puedes utilizar el comando subset() junto con un operador lógico & (and) para 
eliminar las filas en las que el valor de las columnas "ct_score" y "t_score" 
sea mayor que 15.
```{r}
df_test <- subset(df_test, !(df_test$ct_score > 15 & df_test$t_score > 15))
df_test <- subset(df_test, df_test$map != "Cache")
```


Puedes utilizar el comando subset() junto con un operador lógico & (and) para eliminar las filas en las que el valor de las columnas "ct_score" y "t_score" sea mayor que 15Este código utiliza la función colSums() junto con is.na() para contar el número de valores faltantes (NA) en cada columna del dataframe "df_test".

La función is.na() determina si un valor es faltante (NA) y devuelve una matriz lógica con el mismo tamaño que el dataframe de entrada, donde cada valor verdadero indica que el valor en esa posición es faltante.

La función colSums() toma una matriz lógica como entrada y devuelve un vector con la suma de los valores verdaderos (1) en cada columna. En este caso, el código devuelve el número de valores faltantes (NA) en cada columna del dataframe "df_test".
```{r}
colSums(is.na(df_test))
```

```{r}
ggplot(df_test, aes(x = round_winner)) + geom_bar(aes(fill = round_winner)) +
  stat_count(aes(label = ..count..), geom = "text", vjust = 2.0, color = "black", size = 6.5) +
  ggtitle("Rondas ganadas por cada equipo") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("") + 
  ylab("Número de rondas") +
  scale_fill_manual(name = "Ganador ronda", values = c("CT" = "#3288FF", "T" = "#CC5151")) +
  theme(plot.caption = element_text(size=15)) +
  labs(caption = "Fuente: HTLV.org")
```

```{r}
ggplot(df_test, aes(x = round_winner)) + geom_bar(aes(fill = round_winner)) +
  stat_count(aes(label = ..count..), geom = "text", vjust = 2.0, color = "black", size = 5) +
  ggtitle("Rondas ganadas por cada equipo en cada mapa") +
  theme(plot.title = element_text(hjust = 0.5)) +
  xlab("") + 
  ylab("Número de rondas") +
  scale_fill_manual(name = "Ganador ronda", values = c("CT" = "#3288FF", "T" = "#CC5151")) +
  facet_wrap(~map, ncol = 3) + 
  theme(plot.caption = element_text(size=15)) +
  labs(caption = "Fuente: HTLV.org")
```


# Estas son las columnas con todo valor 0:
```{r}
which(colSums(df_test == 0) == nrow(df_test))
```

# Borrar las columnas que tienen todos sus valores "0"
```{r}
df_test <- df_test[, colSums(df_test != 0) > 0]
```

```{r}
dim(df_test)
```
# Hemos conseguido borrar 14 columnas

```{r}
head(df_test)
```

---------------------------------------------------
ETIQUETAR Y CODIFICAR TODOS LOS VALORES CATEGÓRICOS
---------------------------------------------------

# La primera línea convierte la columna especificada (en este caso 'map') a un factor.
```{r}
df_test$map <- as.factor(df_test$map)
```

# La segunda línea convierte el factor a un número.
```{r}
df_test$map <- as.numeric(df_test$map)
```

```{r}
df_test$bomb_planted <- as.factor(df_test$bomb_planted)
df_test$bomb_planted <- as.numeric(df_test$bomb_planted)
```

```{r}
df_test$round_winner <- as.factor(df_test$round_winner)
df_test$round_winner <- as.numeric(df_test$round_winner)
df_test$round_winner <- df_test$round_winner - 1
```

```{r}
head(df_test)
```

```{r}
col <- colnames(df_test)
c <- lapply(col, function(i) cor(df_test$round_winner, df_test[,i]))
```

```{r}
str(df_test)
```

-------------------------------------------------
Normalización de los datos usando Standard Scalar
-------------------------------------------------

La primera línea carga la librería caret.
La segunda línea crea el objeto standscl que se encarga de escalar los datos del dataframe df_test utilizando el metodo preProcess.
```{r}
standscl <- preProcess(df_test, method = c("center", "scale"))
```

La tercera línea se utiliza el objeto standscl para escalar los datos y se almacena en el mismo dataframe df_test.
La cuarta línea convierte df_test a un dataframe de R.
```{r}
df_test <- predict(standscl, df_test)
df_test <- as.data.frame(df_test)
```

La quinta línea se asignan los nombres de las columnas del dataframe df_test a la variable col.
La sexta línea se codifica la variable round_winner utilizando la función as.factor() y as.numeric()
```{r}
colnames(df_test) <- col
df_test$round_winner <- as.factor(df_test$round_winner)
df_test$round_winner <- as.numeric(df_test$round_winner)
```

La séptima línea se utiliza la función head() para ver las primeras filas del dataframe.

Al final del proceso, se tiene un dataframe escalado y con la variable round_winner codificada.
```{r}
head(df_test)
```

```{r}
View(df_test)
```


--------------------------
Dividir los datos en X e Y
--------------------------

```{r}
x <- df_test[ , !(names(df_test) %in% c("round_winner"))]
head(x)
```

```{r}
y <- df_test$round_winner
y <- y-1
head(y)
```

----------------
Train Test Split
----------------

```{r}
split <- createDataPartition(y, p = 0.8, list = FALSE)
x_train <- x[split,]
y_train <- y[split]
x_test <- x[-split,]
y_test <- y[-split]
```

# Modelo 1: Logistic Regression
```{r}
model_1 <- glm(y_train ~ ., data = x_train, family = binomial())
pred_1 <- predict(model_1, x_test, type = "response")
pred_1 <- ifelse(pred_1 > 0.5, 1, 0)
```

```{r}
pred_1 <- as.numeric(pred_1)
pred_1 <- as.factor(pred_1)
y_test <- as.numeric(y_test)
y_test <- as.factor(y_test)
```

```{r}
cr1 <- confusionMatrix(pred_1, y_test)
print(cr1)
```

```{r}
conf_matrix <- as.data.frame(table(pred_1, y_test))

ggplot(data = conf_matrix, aes(x = pred_1, y = y_test)) +
  ggtitle("Modelo 1: Logistic Regression", subtitle = "Confusion Matrix") +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
  theme(plot.title = element_text(size = 20), plot.subtitle = element_text(size = 15)) +
  xlab("Valores originales") + 
  ylab("Valores de test") +
  geom_tile(aes(fill = Freq)) +
  geom_text(aes(label = Freq), color = "black", size = 6, vjust = 0.1, hjust = 0.6) +
  scale_fill_gradient(low = "#E0CB48", high = "#CE93D8")
```

# Modelo 2: Decision Tree

```{r}
library(rpart)
model_2 <- rpart(y_train ~ ., data = x_train, method = "class")
pred_2 <- predict(model_2, x_test, type = "class")
cr2 <- confusionMatrix(pred_2, y_test)
print(cr2)
```


```{r}
conf_matrix_2 <- as.data.frame(table(pred_2, y_test))

ggplot(data = conf_matrix_2, aes(x = pred_2, y = y_test)) +
  ggtitle("Modelo 2: Decision Tree", subtitle = "Confusion Matrix") +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
  theme(plot.title = element_text(size = 20), plot.subtitle = element_text(size = 15)) +
  xlab("Valores originales") + 
  ylab("Valores de test") +
  geom_tile(aes(fill = Freq)) +
  geom_text(aes(label = Freq), color = "black", size = 6, vjust = 0.1, hjust = 0.6) +
  scale_fill_gradient(low = "#86B1D3", high = "#F49541" )
```

```{r}
model_3 <- svm(y_train ~ ., data = x_train, type = "C-classification", kernel = "polynomial")
pred_3 <- predict(model_3, x_test, type = "class")
cr3 <- confusionMatrix(pred_3, y_test)
print(cr3)
```

```{r}
conf_matrix_3 <- as.data.frame(table(pred_3, y_test))

ggplot(data = conf_matrix_3, aes(x = pred_3, y = y_test)) +
  ggtitle("Modelo 3: SVM POLYNOMIAL", subtitle = "Confusion Matrix") +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
  theme(plot.title = element_text(size = 20), plot.subtitle = element_text(size = 15)) +
  xlab("Valores originales") + 
  ylab("Valores de test") +
  geom_tile(aes(fill = Freq)) +
  geom_text(aes(label = Freq), color = "black", size = 6, vjust = 0.1, hjust = 0.6) +
  scale_fill_gradient(low = "#90CAF9", high = "#FF410D" )
```

```{r}
model_4 <- svm(y_train ~ ., data = x_train, type = "C-classification", kernel = "radial")
pred_4 <- predict(model_4, x_test, type = "class")
cr4 <- confusionMatrix(pred_4, y_test)
print(cr4)
```

```{r}
conf_matrix_4 <- as.data.frame(table(pred_4, y_test))

ggplot(data = conf_matrix_4, aes(x = pred_4, y = y_test)) +
  ggtitle("Modelo 3: SVM RADIAL", subtitle = "Confusion Matrix") +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
  theme(plot.title = element_text(size = 20), plot.subtitle = element_text(size = 15)) +
  xlab("Valores originales") + 
  ylab("Valores de test") +
  geom_tile(aes(fill = Freq)) +
  geom_text(aes(label = Freq), color = "black", size = 6, vjust = 0.1, hjust = 0.6) +
  scale_fill_gradient(low = "#90CAF9", high = "#FF410D" )
```

```{r}
model_5 <- svm(y_train ~ ., data = x_train, type = "C-classification", kernel = "sigmoid")
pred_5 <- predict(model_5, x_test, type = "class")
cr5 <- confusionMatrix(pred_5, y_test)
print(cr5)
```

```{r}
conf_matrix_5 <- as.data.frame(table(pred_5, y_test))

ggplot(data = conf_matrix_5, aes(x = pred_5, y = y_test)) +
  ggtitle("Modelo 3: SVM SIGMOID", subtitle = "Confusion Matrix") +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
  theme(plot.title = element_text(size = 20), plot.subtitle = element_text(size = 15)) +
  xlab("Valores originales") + 
  ylab("Valores de test") +
  geom_tile(aes(fill = Freq)) +
  geom_text(aes(label = Freq), color = "black", size = 6, vjust = 0.1, hjust = 0.6) +
  scale_fill_gradient(low = "#90CAF9", high = "#FF410D" )
```





















